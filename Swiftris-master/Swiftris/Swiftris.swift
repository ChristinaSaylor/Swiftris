...
class Swiftris {
    ...
    //Added for restart button so that it doesn't add a new block before restarting the game - Justin
    func removeBlocks(){
        guard let shape = fallingShape
        else {
            return
    ...
    func endGame() {
        //calls setscore & setlevel functions - AJ
        setScore()
        setLevel()
        score = 0
        level = 1
    ...
    //Set highscore if score greater than userDefault - AJ
    func setScore()
    {
    ...
    //set highest level reached if level greater than userDefault - AJ
    func setLevel()
    {
        if NSUserDefaults.standardUserDefaults().objectForKey("highLevel") != nil
        {
            if level > userDefaults.integerForKey("highLevel")
            {
                userDefaults.setInteger(level, forKey: "highLevel")
                userDefaults.synchronize()
            }
        }
            else
